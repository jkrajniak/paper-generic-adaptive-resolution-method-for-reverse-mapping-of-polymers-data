...
lj_cutoff = 1.2
cg_cutoff = 1.2
table_groups = ['A', 'B']  # CG table groups

h5md_group = 'atoms'
filename = 'but.h5'

# Do not to modified lines below.
...
def main():  #NOQA
    ...
    part_prop, all_particles = espressopp.tools.gromacs.genParticleList(
    ...
    if args.coord:
        ...
        try:
            box = h5coord['/particles/{}/box/edges/value'.format(h5md_group)][-1]
        except:
            box = h5coord['/particles/{}/box/edges'.format(h5md_group)]
...
# Define the thermostat
    ...
    system_analysis.add_observable(
        'lj', espressopp.analysis.PotentialEnergy(system, vl_interaction))
    if cg_vl_interaction:
        system_analysis.add_observable(
            'lj-tab', espressopp.analysis.PotentialEnergy(system, cg_vl_interaction))
    for lb, interHarmonic in bondedinteractions.iteritems():
        system_analysis.add_observable(
            'bond_%d' % lb, espressopp.analysis.PotentialEnergy(system, interHarmonic), False)
    for lb, interAngularHarmonic in angleinteractions.iteritems():
        system_analysis.add_observable(
            'angle_%d' % lb, espressopp.analysis.PotentialEnergy(system, interAngularHarmonic),
            False)
    for lb, interDihHarmonic in dihedralinteractions.iteritems():
        system_analysis.add_observable(
            'dihedral_%d' % lb, espressopp.analysis.PotentialEnergy(system, interDihHarmonic),
            False)
    for lb, interaction14 in pairinteractions.iteritems():
        system_analysis.add_observable(
            'lj-14_%d' % lb, espressopp.analysis.PotentialEnergy(system, interaction14), False)
    system_analysis.add_observable('P', pressure_comp, True)
    if coulombinteraction:
        system_analysis.add_observable(
            'coul', espressopp.analysis.PotentialEnergy(system, coulombinteraction), True)
    ...
    k_trj_collect = max(integrator_step, args.trj_collect) / integrator_step
    k_energy_collect = max(integrator_step, args.energy_collect) / integrator_step
