...
def setLennardJonesInteractions(system, input_conf, verletlist, cutoff, nonbonded_params=None, hadress=False, ftpl=None, table_groups=None):
    ...
    type_pairs = sorted({
        tuple(sorted([type_1, type_2])) for type_1, pi in atomtypeparams.iteritems() for type_2, pj in atomtypeparams.iteritems()
...
def setPairInteractions(system, input_conf, cutoff, ftpl=None):
    ...
    for (pid, cross_bonds), pair_list in pairs.iteritems():
        ...
        is_cg = input_conf.atomtypeparams[input_conf.types[pair_list[0][0]-1]]['particletype'] == 'V'
        if is_cg or ftpl is None:
            fpl = espressopp.FixedPairList(system.storage)
        else:
            fpl = espressopp.FixedPairListAdress(system.storage, ftpl)
        fpl.addBonds(pair_list)
        print 'Pair interaction', params, ' num pairs:', len(pair_list), 'sig=', params['sig'], params['eps']
        ...
        if is_cg is None:
            ...
            interaction = espressopp.interaction.FixedPairListAdressLennardJones(system, fpl, pot, is_cg)
        system.addInteraction(interaction)
...
def setDihedralInteractions(system, input_conf, ftpl=None):
    ...
    for (did, cross_dih), dihedrallist in dihedrals.iteritems():
        ...
        if not cross_dih:
            is_cg = None
        dihedralinteraction=dihedraltypeparams[did].createEspressoInteraction(system, fpl, is_cg=is_cg)
